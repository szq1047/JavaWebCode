...
.errorMessage {font-weight:bold; color:red; }
 ...
 <script src="<%=basePath %>calendar1.js"></script>
<script language="javascript">
function GetDateNow(){
	var borrowDate=document.getElementById("loanInfo.borrowDate");
	var returnDate=document.getElementById("loanInfo.returnDate");
	var date = new Date();
	var dateFormat = date.getFullYear() + "-"+ (date.getMonth() + 1) + "-" + date.getDate();
	borrowDate.value=dateFormat;
	date.setDate(date.getDate()+30);
	returnDate.value= date.getFullYear() + "-"+ (date.getMonth() + 1) + "-" + date.getDate();
}
/*验证表单*/
function checkForm() {
    return true; 
...
<table width='100%' cellspacing='1' cellpadding='3' class="tablewidth">
  ...
  <tr>
    <td width=30%>借阅时间:</td>
    <td width=70%><input type="text" readonly id="loanInfo.borrowDate"  name="loanInfo.borrowDate"  onclick="GetDateNow()"/>
    </td>
  </tr>

  <tr>
    <td width=30%>归还时间:</td>
    <td width=70%><input type="text" readonly id="loanInfo.returnDate"  name="loanInfo.returnDate" /></td>
  </tr>
